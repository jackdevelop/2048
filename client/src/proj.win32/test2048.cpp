    ...
    int       nCmdShow)
{
    ...
    return Test2048Player::createAndRun();
}

int Test2048Player::createAndRun(void)
{
    Test2048Player *host = Test2048Player::sharedInstance();
    int ret = host->run();
    Test2048Player::purgeSharedInstance();
    return ret;
}

Test2048Player *Test2048Player::s_sharedInstance = NULL;

Test2048Player *Test2048Player::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new Test2048Player();
    }
    return s_sharedInstance;
}

void Test2048Player::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
Test2048Player::Test2048Player(void)
: m_app(NULL)
, m_hwnd(NULL)
, m_exit(TRUE)
...
int Test2048Player::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "test2048-player");
...
void Test2048Player::loadProjectConfig(void)
{
    vector<string> args;
...
void Test2048Player::createViewMenu(void)
{
...
void Test2048Player::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void Test2048Player::updateWindowTitle(void)
{
}

void Test2048Player::relaunch(void)
{
...
void Test2048Player::writeDebugLog(const char *log)
{
...
// menu callback
void Test2048Player::onFileOpenProject(void)
{
...
void Test2048Player::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"Test2048Player");
...
void Test2048Player::onFileProjectConfig(void)
{
...
void Test2048Player::onFileRelaunch(void)
{
    relaunch();
}

void Test2048Player::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void Test2048Player::onViewChangeFrameSize(int viewMenuID)
{
...
void Test2048Player::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void Test2048Player::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void Test2048Player::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT Test2048Player::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    Test2048Player *host = Test2048Player::sharedInstance();
...
INT_PTR CALLBACK Test2048Player::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
