local FightModel = class("FightModel",cc.mvc.ModelBase)

FightModel.FLASH_EVENT	= "FLASH_EVENT"
FightModel.WIN_EVENT		= "WIN_EVENT"
FightModel.LOST_EVENT	= "LOST_EVENT"
FightModel.MOVE_EVENT	= "MOVE_EVENT"
FightModel.MERGE_EVENT	= "MERGE_EVENT"
FightModel.NOMOVE_EVENT	= "NOMOVE_EVENT"

function FightModel:ctor(properties)
	self.super.ctor(self,properties)
	self.bestScore = 0
end

--重新开始游戏要刷新的数据放这里，不刷新的放在ctor中。
function FightModel:startGame()
	self.currId = 0
	self.currScore = 0
...
-- 由于move、merge和动画延迟执行等原因，getChildrenByTag是不可靠的。
function FightModel:getId()
...
function FightModel:createEmptyPosTable()
	local t = {}
	for i=1,4 do
		for j=1,4 do
...
function FightModel:creatNewCell()
	--创建当前空位表
...
function FightModel:flash(running)
...
function FightModel:checkLost()
	...
	self:dispatchEvent({name=FightModel.LOST_EVENT})
end
...
-- 矩阵变换，用于左移p，左右互换(水平镜像)
function FightModel:leftMap(srcMap)
	local map = {}
	for i=1,4 do
		map[i] = {}
		for j=1,4 do
			map[i][j] = srcMap[5-i][j]
		end
	end
	return map
end
...
-- 矩阵变换，用于上移，行列互换
function FightModel:upMap(srcMap)
	local map = {}
	for i=1,4 do
		map[i] = {}
		for j=1,4 do
...
-- 矩阵变换，用于右移p，左右互换(水平镜像)
function FightModel:rightMap(srcMap)
	local map = {}
	for i=1,4 do
		map[i] = {}
		for j=1,4 do
...
function FightModel:downMap(srcMap)
	local map = {}
	for i=1,4 do
		map[i] = {}
		for j=1,4 do
...
-- 响应玩家的移动事件
function FightModel:move(eventId)
	--由self.map变换成对应map
	local map =	eventId == TOUCH_MOVED_UP and self:upMap(self.map)
		...
		or eventId == TOUCH_MOVED_LEFT and self:leftMap(self.map)
		or self.map

	self.update = false --开始一次新的move前设置，检查是否有移动或合并
	for i=1,4 do
	...
	self.map =	eventId == TOUCH_MOVED_UP and self:upMap(map)
		...
		or eventId == TOUCH_MOVED_LEFT and self:leftMap(map)
		or map

	if self.isWin then
		self:dispatchEvent({
			name=FightModel.WIN_EVENT,
	...
	if self.update then
		...
		self:dispatchEvent({name = FightModel.NOMOVE_EVENT})
	end
end

-- 表内移动算法，dis为移动的距离(格子数)，用于控制View中动画时间。
function  FightModel:moveInMap(des,src,dis)
	self.moveEventParams = {
		name=FightModel.MOVE_EVENT,
...
-- 表内合并算法
function FightModel:mergeInMap(des,src)
	des.v = des.v * 2
	...
	self.mergeEventParams = {
		...
		src=src.id,des=des.id,desTag = des.tag,num=des.v,dis=1
	}
...
--单行移动、合并的算法
function FightModel:lineMerge(arr)
	for cur =2,4 do
		if arr[cur].v ~=0 then
